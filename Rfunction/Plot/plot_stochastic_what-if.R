...
file_secondary <- "~/results_model_analysis_vacc_failure_00/Pertussis-analysys-1.trace"
#######################
...
n_run <- length(main_tr$Time)/length(unique(main_tr$Time))

col_names <- "(InfectCount_a){1}[0-9]{1}"
col_idxs <- c( which(names(main_tr) %in% grep(col_names, names(main_tr), value=T)) )
main_tr <- cbind(main_tr,data.frame(id=unlist(lapply(1:n_run, rep,times=44))))
secondary_tr <- cbind(secondary_tr,data.frame(id=unlist(lapply(1:n_run, rep,times=44))))
main_infects <- lapply(1:n_run,
                       function(x){
...
secondary_infects <- lapply(1:n_run,
                            function(x){
...
ggsave(plot = plt,
       filename = paste0("~/Plots/ViolinPlot_p0",vaccination,".pdf"),
...
ggsave(plot = compare_ecdf,
       filename = paste0("~/Plots/ECDF_p0",vaccination/10,".pdf"),
       dpi = 400,
       width = 14,
       height = 8,
       device = "pdf")
...
ggsave(plot = plt_col,
       filename = paste0("~/Plots/pv_0", vaccination/10, "_", years, "years.pdf"),
       dpi = 400,
       width = 14,
       height = 16,
       device = "pdf")
